...
import { Terminal, Zap, ShoppingCart, Search, Flame } from "lucide-react";
  ...
  return (
    <section id="raffle-grid" className="w-full py-16 bg-rockstar-black relative font-vice border-t border-rockstar-border">
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 pb-4 border-b border-rockstar-border">
          <div>
            <div className="flex items-center gap-2 text-xs font-mono text-rockstar-yellow font-bold uppercase">
              <Terminal className="w-4 h-4 text-rockstar-yellow" /> SELETOR DE BILHETES (1 A 100)
            </div>
        ...
        {/* 100 TICKETS GRID CONTAINER */}
        <div className="bg-rockstar-card border border-rockstar-border rounded-2xl p-4 sm:p-6 shadow-2xl">
            ...
            {filteredTickets.map((ticket) => {
              ...
              return (
                <button
                  ...
                  className={`relative aspect-square flex flex-col items-center justify-center font-mono text-sm sm:text-base font-black rounded-lg border-2 transition-all duration-200 ${
                    isSelected
                ...
                >
                  <span>{String(ticket.number).padStart(2, "0")}</span>

                  {isSold && (
                    <span className="absolute inset-0 flex items-center justify-center text-red-500/80 font-black text-xl select-none">
                      ✕
                    </span>
                  )}
                </button>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-base font-bold text-white">
                  TOTAL: <span className="text-rockstar-yellow">R$ {totalAmount.toFixed(2).replace(".", ",")}</span>
                </div>
              </div>
            </div>
